...
//
// "Updated this script to use the New Input System - Caleb Griffin (Original by Jess Case)" || version 1.0.2
...
public class NewFirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(cameraCanMove)
        {
            yaw = transform.localEulerAngles.y + lookAction.ReadValue<Vector2>().x * mouseSensitivity;

            if (!invertCamera)
            {
                pitch -= mouseSensitivity * lookAction.ReadValue<Vector2>().y;
            }
            else
            {
                // Inverted Y
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && sprintAction.phase == InputActionPhase.Performed && sprintRemaining > 0f && !isSprintCooldown)
            {
...
// Custom Editor
#if UNITY_EDITOR
    [CustomEditor(typeof(NewFirstPersonController)), InitializeOnLoadAttribute]
    public class NewFirstPersonControllerEditor : Editor
    {
    ...
    {
        fpc = (NewFirstPersonController)target;
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("By Jess Case and Caleb Griffin", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
